...
generateHtml(data, '{}_HFSS.html'.format(version_name), 'OrangeRed')

#3D Layout
oDesign = oProject.InsertDesign("HFSS 3D Layout Design", "EMDesign1", "", "")
oEditor = oDesign.SetActiveEditor("Layout")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_3D Layout.html'.format(version_name),'RoyalBlue')

#Circuit
oDesign = oProject.InsertDesign("Circuit Design", "Circuit1", "", "")
oEditor = oDesign.SetActiveEditor("SchematicEditor")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Circuit.html'.format(version_name),'SeaGreen')

#Q3D
oDesign=oProject.InsertDesign("Q3D Extractor", "Q3DDesign1", "", "")
oEditor = oDesign.SetActiveEditor("3D Modeler")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Q3D.html'.format(version_name),'Magenta')

#Maxwell
oDesign=oProject.InsertDesign("Maxwell 3D", "Maxwell3DDesign1", "Magnetostatic", "")
oEditor = oDesign.SetActiveEditor("3D Modeler")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Maxwell.html'.format(version_name),'DarkSalmon')

#Icepak
oDesign=oProject.InsertDesign("Icepak", "IcepakDesign2", "TemperatureAndFlow", "")
oEditor = oDesign.SetActiveEditor("3D Modeler")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Icepak.html'.format(version_name),'GoldenRod')

#Mechanocal
oDesign=oProject.InsertDesign("Mechanical", "MechanicalDesign1", "", "")
oEditor = oDesign.SetActiveEditor("3D Modeler")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Mechanical.html'.format(version_name),'MediumPurple')

#Simplorer
oDesign=oProject.InsertDesign("Twin Builder", "Simplorer1", "", "")
oEditor = oDesign.SetActiveEditor("SchematicEditor")
data=exportFunctions(oDesign, oEditor)
generateHtml(data, '{}_Simplorer.html'.format(version_name),'Sienna')
