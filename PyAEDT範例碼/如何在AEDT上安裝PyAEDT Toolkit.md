...
3. **安裝 PyAEDT** ：使用 pip 進行線上安裝，pip 會根據作業系統自動選擇適當的版本來安裝相關套件。具體安裝指令如下：
...
add_pyaedt_to_aedt(r"/AnsysEM/v261/Linux64/syslib")
```
 
5. **確認安裝完成** ：當安裝完成後，重新啟動 AEDT，所有使用者便都可以在圖形介面中看到 PyAEDT Toolkit 的圖示 (icons)，這樣就實現了多用戶共用的安裝方式。
